...
_DOCKER_IMAGE="blackduckhub/eval:latest"
_HOSTNAME=$(nslookup $(hostname) | grep "Name" | awk '{print $2}')
...
if [ $nr_containers = 0 ]; then
    ...
    STARTUP_CMD="docker run -d  -h $_HOSTNAME --name hub_eval  -p 4181:4181 -p 80:8080 -p 7081:7081 -p 55436:55436 -p 8009:8009 -p 8993:8993 -p 8909:8909 $_DOCKER_IMAGE"
    echo $STARTUP_CMD
    $STARTUP_CMD
fi
